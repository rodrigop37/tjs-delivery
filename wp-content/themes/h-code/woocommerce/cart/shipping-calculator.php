...
<form class="woocommerce-shipping-calculator no-transition" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">

	<a href="#" class="shipping-calculator-button"><?php esc_html_e( 'Calculate Shipping', 'woocommerce' ); ?></a>
	...
	<div class="shipping-calculator-form" style="display:none;">
						...
						<select name="calc_shipping_state" id="calc_shipping_state">
							...
							<?php
								foreach ( $states as $ckey => $cvalue )
									echo '<option value="' . esc_attr( $ckey ) . '" ' . selected( $current_r, $ckey, false ) . '>' . wp_kses( esc_html( $cvalue ), 'woocommerce' ) .'</option>';
							?>
						</select>
					</span><?php
		...
		<p><button type="submit" name="calc_shipping" value="1" class="button highlight-button btn btn-very-small no-margin pull-right"><?php esc_html_e( 'Update Totals', 'woocommerce' ); ?></button></p>
